...
def construct_slater_determinants_2d(basis_size_x, basis_size_y, n_particles):
    ...
    orbitals = [
        (nx, ny, spin)
        for nx in range(basis_size_x)
        for ny in range(basis_size_y)
        for spin in ["up", "down"]
    ]
...
def compute_matrix_element_2d(det_i, det_j, basis_functions, n_particles):
    ...
    if det_i == det_j:
        ...
        kinetic_energy = sum(
            kinetic_energy_integral_2d(nx, ny, basis_functions)
            for (nx, ny, spin) in det_i
        )
        potential_energy = sum(
            potential_energy_integral_2d(nx, ny, basis_functions)
            for (nx, ny, spin) in det_i
        )
        coulomb_energy = sum(
            coulomb_integral_2d(nx1, ny1, nx2, ny2, basis_functions)
            for i, (nx1, ny1, spin1) in enumerate(det_i)
            for j, (nx2, ny2, spin2) in enumerate(det_i)
            if i < j
        )
...
def main():
    ...
    x = np.linspace(-5, 5, 200)  # noqa
    y = np.linspace(-5, 5, 200)  # noqa
    basis_functions = {
        (nx, ny): lambda X, Y, nx=nx, ny=ny: harmonic_oscillator_basis_2d(nx, ny, X, Y)
        for nx in range(basis_size_x)
        for ny in range(basis_size_y)
    }
    det_basis = construct_slater_determinants_2d(
        basis_size_x, basis_size_y, n_particles
    )
    ...
    for i, det_i in enumerate(det_basis):
        for j, det_j in enumerate(det_basis):
            H[i, j] = compute_matrix_element_2d(
                det_i, det_j, basis_functions, n_particles
            )
