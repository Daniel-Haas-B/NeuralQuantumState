...
def hartree_fock(
    ...
    """
    x = np.linspace(-5, 5, 100)  # noqa
    y = np.linspace(-5, 5, 100)  # noqa
    basis_functions = {
            ...
            for nx2 in range(basis_size_x):
                for ny2 in range(basis_size_y):
                    if (nx1 == nx2) and (ny1 == ny2):
                        H_core[
                            nx1 * basis_size_y + ny1, nx2 * basis_size_y + ny2
                        ] = kinetic_energy_integral_2d(
                            nx1, ny1, basis_functions
                        ) + potential_energy_integral_2d(
                            nx1, ny1, basis_functions
                        )
                        ...
                        for ny2 in range(basis_size_y):
                            ...
                            F[idx1, idx2] += sum(
                                ...
                                * (
                                ...
                                for nx3 in range(basis_size_x)
                                for ny3 in range(basis_size_y)
                                for nx4 in range(basis_size_x)
                                for ny4 in range(basis_size_y)
                            )
        return F
...
def main():
    ...
    energies, coeffs = hartree_fock(
        basis_size_x, basis_size_y, n_particles, include_interaction
    )
