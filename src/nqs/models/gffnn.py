...
class GFFNN(WaveFunction):
    ...
    def reinit_positions(self):
        ...
        self.state = State(
            self.r0, self.logprob(self.r0), 0, self.state.delta
        )  # TODO: WILL BREAK IF params is not passed to logprob
    ...
    def ffnn(self, x, params):
        ...
        for i in range(0, len(self._layer_sizes) - 1):
            ...
            # print(f"W{i}", params.get(f"W{i}").shape)
            # print(f"b{i}", params.get(f"b{i}").shape)
