    ...
    for MCcycle in range(NumberMCcycles):
        ...
        for i in range(NumberParticles):
            for j in range(Dimension):
                PositionNew[i, j] = (
                    PositionOld[i, j]
                    + normalvariate(0.0, 1.0) * sqrt(TimeStep)
                    + QuantumForceOld[i, j] * TimeStep * D
                )
            wfnew = rbm(PositionNew, a, b, w)
...
w = np.random.normal(
    loc=0.0, scale=0.001, size=(NumberParticles, Dimension, NumberHidden)
)
...
for i in range(10000):
    coordinates.append(np.random.uniform(-5, 5, (NumberParticles, Dimension)))
