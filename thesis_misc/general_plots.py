...
def psi_cat(q, alpha, phi):
    return (1 / np.sqrt(2)) * (np.exp(-(q - alpha)**2) + np.exp(-(q + alpha)**2) * np.exp(1j * phi))
...
surf = ax.plot_surface(Q, P, W, cmap='BuPu', edgecolor='none', alpha=0.8, linewidth=0.4, color="grey")
...
ax.plot(q, np.full_like(q, 5), psi_q_scaled, color=mycolor, lw=2, label=r'$|\langle q | \psi \rangle|^2$')
